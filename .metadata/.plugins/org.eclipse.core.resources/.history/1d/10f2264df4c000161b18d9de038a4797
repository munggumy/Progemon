...
public abstract class Animation implements IRenderable {
	...
	/** Image used for animation. Must consists of 5 frames per row */
	...
	public void loadAnimationImage(String filePath) {
		...
		/*
		 * if(name.equals("Flamethrower")) { File file = new
		 * File("load/img/skill/" + name + "/all.png"); Image image = new
		 * Image(file.toURI().toString()); animation = new Animation(image, 16);
		 * }
		 */
		/*
		 * File file = new File(filePath); this.image = new
		 * Image(file.toURI().toString()); frameNumber = 16;
		 */
	}
