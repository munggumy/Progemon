...
public class QueueBox implements IRenderable {
	...
	private static List<Pokemon> pokemonsOnMap = new ArrayList<Pokemon>(GUIFightGameManager.getFightMap().getPokemonsOnMap());
	...
	private void move(){
		if(moveTimeCounter == MOVE_TIME){
			...
			for (Pokemon pokemon : pokemonsOnQueue) {
				delta[pokemonsOnQueue.indexOf(pokemon)][1] += MOVE_RATE * (pokemonsOnMap.indexOf(pokemon) - pokemonsOnQueue.indexOf(pokemon));
			}
			moveTimeCounter++;
		}
	}
