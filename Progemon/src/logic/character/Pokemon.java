...
public class Pokemon implements Cloneable, IRenderable {
	...
	public void attack(Pokemon p, int selectedSkill) {
		double damage = ((((2 * level + 10) * activeSkills.get(selectedSkill).getPower() * current.attackStat
				/ p.current.defenceStat) * 0.002) + 2) * RandomUtility.randomPercent(85, 100);
		p.changeHP(-damage);
		...
		System.out.println(this.getOwner().getName() + "'s " + this.getName() + " attacked " + p.getOwner().getName()
				+ "'s " + p.getName() + " with " + activeSkills.get(selectedSkill).getName() + "!");
		for (ActiveSkill activeSkill : activeSkills) {
			System.out.print(activeSkill.getName() + ", ");
		}
	...
	public void findBlocksAround(int range, Filter filter) {
		this.paths.clear();
		findBlocksAroundRecursive(range, new PathNode(this.getCurrentFightTerrain()), this.getCurrentFightTerrain(),
